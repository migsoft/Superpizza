...
*------------------------------------------------------------*
    ...
    define window form_main;
        ...
        width 170
        height 080
        caption 'F5    Venda Delivery'              //                       caption 'F5'+CRLF+'Venda'+CRLF+'Delivery'
        multiline .T.
...
static function cria_dbf_cdx()
    ...
    if .not. file(path_dbf+'cli_dc.cdx')
        if open_dbf('cli_dc','detalhamento_compras',.F.)
            index on str(id_cliente,6)+dtos(data)+hora tag id3 to (path_dbf)+'cli_dc.cdx'
    ...
    if .not. file(path_dbf+'prodcomp.cdx')
        if open_dbf('prodcomp','produto_composto',.F.)
            index on id_produto tag id3_produto to (path_dbf)+'prodcomp.cdx'
        endif
        dbclosearea()
    endif
    ...
    if .not. file(path_dbf+'caixa.cdx')
        if open_dbf('caixa','caixa',.F.)
            index on (caixa->id) tag (caixa->id) to (path_dbf)+'caixa.cdx'
    ...
    if .not. file(path_dbf+'movban.cdx')
        if open_dbf('movban','movimento_bancario',.F.)
            index on (movimento_bancario->id) tag (movimento_bancario->id) to (path_dbf)+'movban.cdx'
    ...
    if .not. file(path_dbf+'cpag.cdx')
        if open_dbf('cpag','contas_pagar',.F.)
            index on (contas_pagar->id) tag (contas_pagar->id) to (path_dbf)+'cpag.cdx'
    ...
    if .not. file(path_dbf+'crec.cdx')
        if open_dbf('crec','contas_receber',.F.)
            index on (contas_receber->id) tag (contas_receber->id) to (path_dbf)+'crec.cdx'
...
*------------------------------------------------------------*
    ...
    if .not. file(path_dbf+'tmp_pza.cdx')
        if open_dbf('tmp_pza','temp_pizzas',.F.)
            cTag := 'id'
            index on (temp_pizzas->id_mesa) tag &(cTag) to (path_dbf)+'tmp_pza.cdx'
        endif
        dbclosearea()
    endif
*------------------------------------------------------------*
...
*------------------------------------------------------------*
    ...
    if .not. file(path_dbf+'tmp_prd.cdx')
        if open_dbf('tmp_prd','temp_produtos',.F.)
            cTag := 'id'
            index on (temp_produtos->id_mesa) tag &(cTag) to (path_dbf)+'tmp_prd.cdx'
        endif
        dbclosearea()
    endif
*------------------------------------------------------------*
...
static function open_dbf_cdx()
    ...
    if l_demo
        if date() > _data_limite
            msgstop('O período de avaliação deste programa foi encerrado. Caso tenha gostado e deseje aquirir,'+;
                ' por favor ligue para os telefones que estão na tela ou envie e-mail solicitando uma cópia definitiva,'+;
                ' com direito a suporte e atualizações, ou então, para manifestar sua opinião, críticas e sugestões serão analisadas.'+;
                ' Muito obrigado - equipe de desenvolvimento.','Atenção')
