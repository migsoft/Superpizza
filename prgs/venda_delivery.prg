...
function venda_delivery()
    ...
    define window form_delivery;
        ...
        @ 020,170 button botao_cadastrar_cliente;
        ...
        @ 403,410 button botao_fecha_pizza;
            parent form_delivery;
            caption 'Pizza montada';
            width 120 height 35;
            picture 'adicionar';
            action fecha_montagem_pizza()
        @ 403,535 button botao_exclui_pizza;
            parent form_delivery;
        ...
        @ 520,(410 + n_largura_grid + 265) button botao_cupom;
            parent form_delivery;
            caption '';
            width 40 height 40;
        ...
        @ 545,410 label label_pedido;
            ...
            fontcolor _preto_001;
            transparent
        @ 540,585 button botao_exclui_item_pedido;
            parent form_delivery;
            caption 'excluir item pedido';
            width 130 height 25;                    //                        fontcolor RED;
...
static function separa_pizza()
    ...
    while .not. eof()
        if produtos->pizza
            add item {produtos->codigo,alltrim(produtos->nome_longo),alltrim(trans(produtos->val_tm_001,'@E 99,999.99')),;
                alltrim(trans(produtos->val_tm_002,'@E 99,999.99')),alltrim(trans(produtos->val_tm_003,'@E 99,999.99')),;
                alltrim(trans(produtos->val_tm_004,'@E 99,999.99')),alltrim(trans(produtos->val_tm_005,'@E 99,999.99')),;
                alltrim(trans(produtos->val_tm_006,'@E 99,999.99'))} to grid_pizzas of form_delivery
        endif
        produtos->(dbskip())
    end
...
static function monta_informacao_pizza()
    ...
    while .not. eof()
        ...
        if found()
            add item {produtos->codigo,alltrim(produtos->nome_longo)+iif(produtos->promocao,' (promoção)',''),trans(produtos->val_tm_001,'@E 99,999.99'),;
                trans(produtos->val_tm_002,'@E 99,999.99'),trans(produtos->val_tm_003,'@E 99,999.99'),trans(produtos->val_tm_004,'@E 99,999.99'),;
                trans(produtos->val_tm_005,'@E 99,999.99'),trans(produtos->val_tm_006,'@E 99,999.99')} to grid_finaliza_pizza of form_finaliza_pizza
        endif
        dbselectarea('tmp_pizza')
        tmp_pizza->(dbskip())
    end
...
static function adiciona_montagem()
    ...
    sele montagem
    append blank
    //replace ( montagem->id ) with &(x_codigo)
    ( montagem->id ) := x_codigo
...
static function transfere_produto_pedido(p_codigo,p_nome,p_unitario)
    ...
    //replace (temp_vendas->id) with alltrim(str(hb_random(0111222233,9999999889)))
    (temp_vendas->id) := alltrim(str(hb_random(0111222233,9999999889)))
    replace tipo with 2
